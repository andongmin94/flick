...
export function getActiveSiteConfig() { return getActiveRule(); }
...
export function runPreHook() {
  const r = getActiveRule();
  if (r?.prePrepare) {
    try { r.prePrepare(); } catch (e) { console.warn('[flick pre hook error]', e); }
  }
}
export function runPostMountedHook() {
  const r = getActiveRule();
  if (r?.postShortsMounted) {
    try { r.postShortsMounted(); } catch (e) { console.warn('[flick post hook error]', e); }
  }
}
