...
(function (ns) {
  ns.extractPost = function () {
    ...
    function sanitizeTitle(t) {
      ...
      (site.titleSanitizers || []).forEach(([re, rep]) => {
        s = s.replace(re, rep);
      });
    ...
    function pickTitle() {
      ...
      selectorList.forEach((sel) => {
        document.querySelectorAll(sel).forEach((el) => {
          ...
          const punctRatio =
            (txt.match(/[.,!?]/g) || []).length / Math.max(1, len);
          const score = len + hasKo - punctRatio * 10;
          candidates.push({ txt, score, len });
        });
      });
      const og = sanitizeTitle(
        document
          .querySelector('meta[property="og:title"]')
          ?.getAttribute("content") || ""
      );
      if (og && !seen.has(og))
        candidates.push({ txt: og, score: og.length + 15, len: og.length });
    ...
    let contentEl = document.querySelector(site.contentSelector || "body");
    if(site.strictArticleRoot){
      // contentEl 안 우선, 없으면 문서 전역 첫 article
      const a = contentEl && contentEl.querySelector('article');
      if(a) contentEl = a; else {
        const globalA = document.querySelector('article');
        if(globalA) contentEl = globalA;
      }
    }
    let blocks = [];
    ...
    const nodes = contentEl.querySelectorAll(
      "p, img, figure, blockquote, pre, h1, h2, h3, h4, h5, h6, li, div, section, dl, dt, dd"
    );
    nodes.forEach((node) => {
      const tag = node.tagName;
      if (site.skipClosest && node.closest && node.closest(site.skipClosest))
        return;
      if (tag === "IMG") {
        const src =
        ...
        if (!plain) {
          if (DEBUG) console.debug("[S2S skip empty]", node);
          return;
        }
        ...
        if (
          ...
          if (
            node.querySelector(
              "p, img, figure, blockquote, pre, h1, h2, h3, h4, h5, h6, li, dl, dt, dd"
            )
          ) {
            if (DEBUG) console.debug("[S2S skip wrapper]", node);
            return;
          }
        }
    ...
    for (const b of blocks) {
      ...
      if (textOnly.length === 0) {
        if (DEBUG) console.debug("[S2S drop empty]", b);
        continue;
      }
      if (textOnly.length === 1 && /[\p{P}\p{S}]/u.test(textOnly)) {
        if (DEBUG) console.debug("[S2S drop lone punct]", textOnly);
        continue;
      }
      // 즉시 중복 (same html next to each other) 언제나 제거
      if (rawHtml === lastHtml) {
        if (DEBUG)
          console.debug("[S2S drop immediate dup]", textOnly.slice(0, 60));
        continue;
      }
      const norm = normalizeText(rawHtml);
      if (norm.length >= 12) {
        if (seenNorm.has(norm)) {
          if (DEBUG) console.debug("[S2S drop seen norm]", norm.slice(0, 60));
          continue;
        }
        seenNorm.add(norm);
      }
