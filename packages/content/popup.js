  ...
  if (openOptionsBtn) {
    openOptionsBtn.addEventListener("click", () => {
      if (chrome.runtime.openOptionsPage) chrome.runtime.openOptionsPage();
        ...
        chrome.tabs.create({
          url: chrome.runtime.getURL("content/options.html"),
        });
    });
  }
        ...
        chrome.scripting.executeScript({
          ...
          func: () => {
            try {
              ...
              if (
                window.FLICK &&
                window.FLICK.isSupportedArticle &&
                window.FLICK.isSupportedArticle()
              ) {
                const open = !!document.querySelector(".flick-wrap-injected");
                if (open && window.FLICK.closeShorts)
                  window.FLICK.closeShorts();
                else if (!open) {
                  const data =
                    window.FLICK.extractPost && window.FLICK.extractPost();
