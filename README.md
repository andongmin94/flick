...
유머 / 커뮤니티 게시글을 한 번에 **YouTube Shorts 스타일(세로 스크롤 미디어 뷰어)** 로 재구성해 주는 브라우저 확장(Chrome Manifest v3)입니다. 지원 게시글 페이지에서 자동으로 작은 토글 배지가 나타나며, 클릭 또는 단축키(F4)로 "쇼츠" 패널을 열고 닫을 수 있습니다.
...
```
packages/
	package.json         # 확장(콘텐츠 스크립트) 빌드 설정
	vite.config.js       # 단일 entry -> bundle.js / bundle.css
	src/
...
docs/                  # VitePress 기반 외부 문서 사이트 소스 (별도: ChatView 문맥 혼재)
README.md              # (현재 파일)
```

> 참고: `docs/` 디렉터리는 초기 별도 앱(Desktop ChatView) 문서와 혼재한 상태로, 추후 정리/분리 권장.
...
`LICENSE.md` (명시된 라이선스) 를 따릅니다. 상표/서비스 로고 등 별도 자산은 저작권자가 소유합니다.
...
> 유지 관리 참고: `docs/` 디렉터리는 별도 Electron ChatView 문서 잔존물이 포함되어 있어 향후 분리/정리 필요.

즐거운 사용 되세요! 피드백 환영합니다 🙌
