...
export default function ContentBody({
  ...
  return (
    <AnimatePresence mode="wait">
      {isLoading ? (
        <div className="flex h-[75.5vh] w-[26.6vw] items-center justify-center pb-10 text-4xl font-bold">
          <style>{`::-webkit-scrollbar { display: none; }`}</style>
          로딩 중...
        </div>
      ) : (
        <div className="scrollbar-thin scrollbar-thumb-primary scrollbar-track-secondary h-[75.5vh] w-[26.6vw] overflow-y-auto">
          <style>{`::-webkit-scrollbar { display: none; }`}</style>
          <motion.div
